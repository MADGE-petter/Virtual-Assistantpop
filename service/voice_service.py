...
class VoiceService:
    ...
    def _idle_check_loop(self):
        """Background loop checking for idle timeout."""
        while not self._stop_idle_event.is_set():
            if self.is_sleeping or self.is_waiting_for_wake:
                time.sleep(1.0)
                continue
            ...
            if time_since_interaction >= self.idle_timeout_seconds:
                print(f"[VoiceService] Idle timeout ({self.idle_timeout_seconds}s) - going to sleep")
